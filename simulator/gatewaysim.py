...
# ─── Farm Identity ───────────────────────────────────────────────
ORG_ID    = os.environ.get("SIM_ORG_ID",  "org_sunrise")
FARM_ID   = os.environ.get("SIM_FARM_ID", "farm_01")


# ════════════════════════════════════════════════════════════════
# Sensor Models — Realistic Behaviour
# ════════════════════════════════════════════════════════════════
...
def simulate_temperature(base: float = 24.0, amplitude: float = 4.0) -> float:
    ...
    Base 24°C ± 4°C + small Gaussian noise.
    Healthy range: 18–32°C
    """
    h = get_hour_fraction()
    # Peak at hour 14 → phase shift: (14/24)*2π
...
class GatewaySimulator:
    def __init__(self, device_id: str, interval: int = 60):
        ...
        self.seq        = 0
        self.state      = {"nh3": random.uniform(8.0, 18.0)}  # each device starts at different NH3
        self.client     = None
    ...
    def _build_payload(self, is_buffered: bool = False, override_ts: int = None) -> dict:
        self.seq += 1
        return {
            "schema_version": "1.0",
            "org_id":         ORG_ID,
    ...
    def connect(self):
        ...
        self.client.loop_start()   # non-blocking — runs network loop in background thread
    ...
    def publish_once(self):
        ...
        print(f"[{self.device_id}] 📡 Sending: temp={payload['readings']['temperature']}°C "
              f"hum={payload['readings']['humidity']}% "
              f"lux={payload['readings']['lux']} "
    ...
    def replay_buffer(self, buffer_file: str):
        ...
        The original sent_ts is preserved — this is what triggers the
        'buffered data' routing fork in Sprint 3.
        """
...
def main():
    ...
    if args.replay:
        ...
        device_id = args.device_id or "gw_lora_001"
